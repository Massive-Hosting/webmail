/** Dialog for sharing a calendar with other users */

import React, { useState, useCallback, useEffect } from "react";
import * as Dialog from "@radix-ui/react-dialog";
import { X, Trash2, Users } from "lucide-react";
import { useTranslation } from "react-i18next";
import { updateCalendar } from "@/api/calendar.ts";
import { toast } from "sonner";
import { useQueryClient } from "@tanstack/react-query";
import type { Calendar, CalendarSharePermission } from "@/types/calendar.ts";
...
function permissionToLevel(perm: CalendarSharePermission): PermissionLevel {
  return perm.mayUpdateItems ? "edit" : "view";
}

function levelToPermission(level: PermissionLevel): CalendarSharePermission {
  if (level === "edit") {
    return { mayReadItems: true, mayUpdateItems: true, mayDelete: true };
  }
  return { mayReadItems: true };
}

export const ShareCalendarDialog = React.memo(function ShareCalendarDialog({
  open,
  onOpenChange,
  calendar,
  ...
  // Sync shares from calendar prop when dialog opens
  useEffect(() => {
    if (open && calendar.shareWith) {
      const entries: ShareEntry[] = Object.entries(calendar.shareWith).map(
        ([email, perm]) => ({
          email,
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const shareWith: Record<string, CalendarSharePermission> = {};
      ...
      toast.success(t("calendar.shareSaved"));
      onOpenChange(false);
    } catch (err) {
      toast.error(
        `Failed to save sharing settings: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setSaving(false);
    }
  }, [shares, calendar.id, queryClient, t, onOpenChange]);
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={t("calendar.addPerson")}
              ...
              >
                <option value="view">{t("calendar.canView")}</option>
                <option value="edit">{t("calendar.canEdit")}</option>
              </select>
              <button
