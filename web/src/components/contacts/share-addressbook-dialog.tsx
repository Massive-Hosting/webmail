/** Dialog for sharing an address book with other users */
...
import { updateAddressBook } from "@/api/contacts.ts";
import { toast } from "sonner";
import { useQueryClient } from "@tanstack/react-query";
import type { AddressBook, AddressBookSharePermission } from "@/types/contacts.ts";
...
function permissionToLevel(perm: AddressBookSharePermission): PermissionLevel {
  return perm.mayUpdateItems ? "edit" : "view";
}

function levelToPermission(level: PermissionLevel): AddressBookSharePermission {
  if (level === "edit") {
...
export const ShareAddressBookDialog = React.memo(function ShareAddressBookDialog({
  open,
  onOpenChange,
  addressBook,
}: ShareAddressBookDialogProps) {
  ...
  useEffect(() => {
    if (open && addressBook.shareWith) {
      const entries: ShareEntry[] = Object.entries(addressBook.shareWith).map(
        ([email, perm]) => ({
          email,
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const shareWith: Record<string, AddressBookSharePermission> = {};
      for (const share of shares) {
        shareWith[share.email] = levelToPermission(share.permission);
      }
      await updateAddressBook(addressBook.id, {
  ...
  }, [shares, addressBook.id, queryClient, t, onOpenChange]);
